...
void ShowAnimal::addShowDate(const Date& d)
{
    ...
    if(this->birthday <= d)
    {
        for(int i = 0; i < this->showCnt; i++)
        {
            if(*(this->showDates[i]) == d)  // 這個指標指到的東西是不是 d
                return;            
        }
...
// better implementation: template
// return the index of the first display animal in "animals" whose ID matches "id" 
int findDisplayTargetByID(DisplayAnimal* animals[], int cnt, int id);

// return the index of the first show animal in "animals" whose ID matches "id" 
int findShowTargetByID(ShowAnimal* animals[], int cnt, int id);

// return the index of the first display animal in "animals" whose name matches "name"
int findDisplayTargetByName(DisplayAnimal* animals[], int cnt, const string& name);

// return the index of the first show animal in "animals" whose name matches "name"
...
int main()
{
    ...
    for(int i = 0; i < t; i++)
    {
        ...
        else if(task == 'S')
        {
            ...
            int aID = findDisplayTargetByName(displayAnimals, displayAnimalCnt, name);
        ...
        {
            cin >> id >> name;
            int aID = findDisplayTargetByID(displayAnimals, displayAnimalCnt, id);
            if(aID != NOT_FOUND_CODE)
                displayAnimals[aID]->setName(name);
            else
            {
                aID = findShowTargetByID(showAnimals, showAnimalCnt, id);
                if(aID != NOT_FOUND_CODE)
        ...
        else if(task == 'A')
        {
            ...
            int aID = findShowTargetByName(showAnimals, showAnimalCnt, name);
            if(aID != NOT_FOUND_CODE)
    ...
    int toPrintID = findShowTargetByName(showAnimals, showAnimalCnt, name);
